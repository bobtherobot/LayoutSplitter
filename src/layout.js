...
this.jbeeb.utils.Layout = (function() {
    ...
    function ripDoc() {
		...
		for (var i = 0; i < all.length; i++) {
			...
			var id = elem.id = elem.id || "layout_uuid_" + (uuid++);
			var info = parseDataset(elem);
			harvestInfo[id] = info;

			if ( ! shouldReset ) {
                shouldReset = info.reset;
            }
		}

        for (var i = 0; i < all.length; i++) {
			var elem = all[i];
			var info = harvestInfo[elem.id];
			...
			if( ! shouldReset ){
				info.size = retoreList[elem.id] || info.size;
			}
			
            var numUnits = parseNumUnits(info.size);
            var numUnitsMin = parseNumUnits(info.min);
            ...
            var obj = {
                ...
                units: numUnits.units
                    ...
                    ,
                clickToggle: ! info.noclicktoggle,
                fixed: info.fixed,
                toggleState: false
                    //, openSize : 
                    ,
				...
				sizeSource : numUnits.num,
                color: info.color,
                //remember: datasetExistBool(elem, "remember"),
                resizable: ! info.noresize,
                canScroll: ! info.noscroll,
                onResizeComplete: info.onresizecomplete // always lower-cased
                    ,
                onResize: info.onresize // always lower-cased
            };
    ...
    function prepSizes(obj, fromResize) {
        ...
        if (kids) {
            ...
            var diff_w = 0;
            var diff_h = 0;
            // var to_w = Math.floor(obj.parent.w || 0);
            // var to_h = Math.floor(obj.parent.h || 0);
			// sum_w = Math.floor(sum_w);
			// sum_h = Math.floor(sum_h);
            ...
            if (obj.row) {
				...
				if (diff_h < 0) {
                    ...
                    if (diff_h < 0) {
                        ...
                        if (fromResize) {
							...
							// // see if only oen size is set:
							// var countSourceSize = 0;
							// for (var i = 0; i < kids.length; i++) {
							// 	if(kids[i].sizeSource){
							// 		countSourceSize++
							// 	}
							// }

							// if(countSourceSize === 1){
				...
				//if (lastObj && lastObj.last && lastObj.fixed) {
				//	sum = sum - (lastObj.prev.h - lastObj.prev.h_orig);
				//	lastObj = lastObj.prev;
				//}
				//con sole.log("to_h - sum", to_h - sum)
	...
	function parseDataset(elem){
		var dataset = elem.dataset.layout;
		var Adataset = dataset.split(",");
		var info = {};
		for(var i=0; i<Adataset.length; i++){
			var key = Adataset[i].trim().toLowerCase().replace(/ /g, "");
			var val;
			if( /=/.test(key) ){
				var Aval = key.split("=");
				key = Aval[0]
