/*
OPTIONS:
col|row (required)	Which direction to split (vertical="col" horizontal="row". 
...
noscroll			When set scrolling will be disabled

Options are added to DIV tags using the data- prefix.
Example:
<div data-row data-size=1 data-min="300" data-resizeable=1 data-noscroll data-onresizecomplete="myFunction" data-onresize="myFunction"></div>
...
jbeeb.LayoutSplitter = (function(){
	...
	mask.classList.add("layout-mask");

	// Measure the splitter
	var temp = document.createElement("div");
	temp.classList.add("layout-col");
	...
	var previousSize = 0;
	var right = 0;
	var all = document.querySelectorAll("[data-col], [data-row]");
	...
	for(var i=0; i<all.length; i++){
		...
		while( isTrue(container.dataset.row) || isTrue(container.dataset.col) ){
		...
		var row = isTrue(elem.dataset.row); // == "row" ? true : false;
		...
		if( isTrue(elem.dataset.noscroll) ){
	...
	for(var i=0; i<cells.length; i++){
		...
		if( ! obj.amRoot){
			...
			if(willSplit){
				...
				if(row){
					splitter.classList.add("layout-row");
					splitter.style.top = obj.max - offset + "px";
				} else {

					splitter.classList.add("layout-col");
				...
				//if(obj.resizable){
					...
					if(row){
						splitterGrabber.classList.add("layout-row-grabber");
						splitter.appendChild(splitterGrabber);
					} else {
						splitterGrabber.classList.add("layout-col-grabber");
						splitter.appendChild(splitterGrabber);
					}
		...
		while(prevObj) {
			...
			//if(prevObj) {
				if ( prevObj.resizable && pos <= moveObj.min ) {
					moveTo( prevObj, pos - splitter_size - 16 );
				}
		...
		if (! suspend ){
				...
				while(nextObj) {
					if(nextObj.resizable && pos > nextObj.max){
						moveTo(nextObj, pos + splitter_size + 16);
					}
	...
	function resize(obj){
		...
		for(var i=0; i<rootList.length; i++){
			...
			if(diff_w || diff_h){
				...
				resizeLast(obj, w, h, diff_w, diff_h);
			}

		}
	}

	function resizeLast(obj, w, h, diff_w, diff_h){
		...
		for(var i=0; i<kids.length; i++) {
			...
			if(kid.children){
				resizeLast(kid, w, h, diff_w, diff_h)
			}
		}
	}
