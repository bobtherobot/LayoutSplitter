...
this.jbeeb.utils.Layout = (function() {
	...
	function parseDataset(elem){
		var info = elem.dataset;
		var Ainfo = info.split(";");
		for(var i=0; i<Ainfo.length; i++){
			var key = Ainfo[i].trim().toLowerCase().replace(/ /g, "");
			var val;
			if( /=/.test(key) ){
				var Aval = key.split("=");
				key = Aval[0]
