...
- __data-hidden__		(optional) - hides / disables the panel (use jbeeb.utils.Layout.hide / show)
- __data-showoverflow__ (optional) - Allows children to display outside of the panel boundaries. 
...
this.jbeeb.utils.Layout = (function() {
	...
	function tryToRip(){
		...
		if( okToRip() ) {
			...
			tryToRipCount++;
			if(tryToRipCount > tryToRipCountMax){
				console.log("tryToRipCountMax");
				ripItGood(true);
			} else {
				setTimeout(tryToRip, 100);
			}
			
		}
	}

	function ripItGood(safe){
	...
	function okToRip(){
		...
		for (var i = 0; i < all.length; i++) {
			...
			if( ! containerDims.height && ! containerDims.width){
				return false;
			}
	...
	function ripDoc() {
		...
		for (var i = 0; i < all.length; i++) {
			...
			var specSize = elem.dataset.size;

			var numUnits = parseNumUnits(specSize);
			...
			var hidden = datasetExistBool(elem, "hidden");
			...
			var obj = {
				...
				showOverflow: datasetExistBool(elem, "showoverflow"),
				hidden: hidden,
				toggleState: false
					//, openSize : 
					,
			...
			if (!obj.canScroll && !obj.showOverflow) {
	 ...
	 * 			con sole.log("didResizeComplete");
	 *  	}
	...
	/**
	 * There may be more than one root item. (When there's a disconnect climbing the tree until no data-layout.)
	 * @private 
	 * @method prepRoots
	 * @param {} fromResize
	 */
	...
	function positionPanels() {
		...
		for (var i = 0; i < cells.length; i++) {
			...
			if (!obj.amRoot) {
				...
				if (obj.hidden) {
					if (obj.prev) {
						if (obj.prev.splitterElem) {
							obj.prev.splitterElem.style.display = "none";
						}
					}
					if (obj.splitterElem) {
						obj.splitterElem.style.display = "none";
					}
	...
	function finishMove() {
		...
		if (!didMove && moveObj.clickToggle) {
			...
			// Front edge elements won't have a handle (handles go onto previous containers)
			if (moveObj.splitterHandleElem) {
	...
	function resizeLast(obj, diff_w, diff_h) {
		...
		for (var i = 0; i < kids.length; i++) {
			...
			if (kid.last) {
				...
				//
				// NO: When there are > 2 panels, assume the last panel is fixed to the edge of the window 
				// and shrinks expands previous panels.
				//
				// YES: When there are > 2 panels, the last panel is fixed to the edge of the window 
				// and shrinks expands previous panels ONLY WHEN the last panel is flagged as "fixed".
				//
				...
				var mover;
				if ( kid.fixed && kid.prev ) {
					mover = kid.prev;
				} else {
					if (count < 2) { // one or two panels
						mover = kid;
					} else { // more than 2 panels
						mover = kid;
					}
				}

				//var mover = (count < 2 || kid.fixed) ? kid.prev : kid;
				// NO: var mover = (count > 1 || kid.fixed) ? kid.prev : kid;
...
console.log("id", id, size);
	...
	function hide(elem) {
		...
		if (obj) {
			...
			resizePanel(obj.prev.elem, pctOfParent(resizeObj, 1) - 2); // can't go over max width, ignored otherwise.
			//resizePanel(resizeObj.elem, pctOfParent(resizeObj, 1));
			//expand(resizeObj.elem);
			//var px = parseFloat(obj.defaultSize)  / 100;
			//resizePanel(resizeObj.elem, pctOfParent(obj, 1) )
		}
	...
	function show(elem) {
		...
		if (obj) {
			if (obj.prev) {
				if (obj.prev.splitterElem) {
					obj.prev.splitterElem.style.display = "inline-block";
				}
			}
			if (obj.splitterElem) {
				obj.splitterElem.style.display = "inline-block";
			}
			obj.elem.style.display = "inline-block";
			...
			//var px = parseFloat(obj.defaultSize)  / 100;
			resizePanel(resizeObj.elem, pctOfParent(obj, 0.5))
		}

	}

	function pctOfParent(obj, pct) {
		var parent = obj.parentElem;

		var containerDims = parent.getBoundingClientRect();
		var px = obj.row ? containerDims.height : containerDims.width;
		return px * pct;
...
// Must wait for CSS and everything to load before parsing the layour.
window.addEventListener("load", jbeeb.utils.Layout.init);
